...
class MHAttention(nn.Module):
    ...
    def __init__(
            ...
            # --- NEW ---
            use_sink: bool = False,
            sink_size: int = 1,  # number of sink anchors
    ):
        ...
        if self.use_sink:
            # learned sink key/value vectors
            self.sink_K = nn.Parameter(
                torch.randn(1, self.n_heads, self.sink_size,
                            embed_dim // n_heads))
        ...
        if local_attention_size is not None and max_cache_size > 0:
            self.max_cache_size = local_attention_size
    ...
    def forward(self, q, k, v, cache_index: int):
        ...
        # ----------- ATTENTION SINK K/V PREPEND ------------
        if self.use_sink:
            sink_K = self.sink_K.expand(full_k.shape[0], -1, -1, -1)
            sink_V = self.sink_V.expand(full_v.shape[0], -1, -1, -1)

            full_k = torch.cat([sink_K, full_k], dim=2)
            full_v = torch.cat([sink_V, full_v], dim=2)

            # ----------------------------------------------------

            ## ADD to mask TODO
            if attn_mask is not None:
                Tq = attn_mask.shape[0]  # query length
                Tk = attn_mask.shape[1]  # original key length

                # new mask: (Tq, sink_size + Tk)
                new_mask = torch.zeros((Tq, self.sink_size + Tk),
                                       device=attn_mask.device,
                                       dtype=attn_mask.dtype)

                new_mask[:, self.sink_size:] = attn_mask
