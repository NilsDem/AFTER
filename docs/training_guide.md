...
__AFTER__ is a diffusion-based generative model that generates audio by blending two sources: one timbre representation, and one stream (either audio or MIDI) that shapes the signal structure (melody, dynamics) over time.

<p align="center"><img src="principle.png" alt="drawing" width="500"/></p>


AFTER operates in the latent space of neural audio codec instead of generating raw audio waveform. The codec task is to compression the high-dimensional audio into compact and informative embeddings. This enables the diffusion to model to run efficiently and to focus on semantic information rather than high-frequency details.

During training, AFTER learns to map this latent input into separated timbre and structure representations using two encoders. Those representations are passed to a diffusion decoder that generates latent codes, further decoded back to audio by the neural codec.

The training is done in two phases : first learning a timbre representation, then learning the structure and diffusion decoder. 
...
Once the timbre representation is learned, we train the structure encoder and diffusion model with a diffusion autoencoder framework. This means that the diffusion model must reconstruct a target signal from a noisy version, conditionned on timbre and structure representations. The timbre encoder is frozen during this phase.
...
- if `--adv` is too low: structure leaks timbre, transfer becomes weak or inconsistent
- if `--adv` is too high: the model loses local fidelity, pitch and rhythm tracking degrade
...
Alternatively, you can use a pretrained model. RAVE models are a common choice, but they are often strongly regularized to enable timbre transfer, which can lead to imperfect reconstruction. In the context of AFTER where transfer is done by the diffusion model, this will unecessarly limit the overall audio quality.
